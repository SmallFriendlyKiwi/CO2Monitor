...
#define HTTP_PORT 80
  ...
  ESPAsync_WMParameter* neopixelMatrixDataParam;
  ESPAsync_WMParameter* featherMatrixDataParam;
  ESPAsync_WMParameter* featherMatrixClockParam;
  ESPAsync_WMParameter* matrixColumnsParam;
  ESPAsync_WMParameter* matrixRowsParam;
  ESPAsync_WMParameter* matrixLayoutParam;
  ...
  void setupWifiManager(ESPAsync_WiFiManager* wifiManager) {
    ...
    sprintf(neopixelMatrixData, "%u", config.neopixelMatrixData);
    sprintf(featherMatrixData, "%u", config.featherMatrixData);
    sprintf(featherMatrixClock, "%u", config.featherMatrixClock);
    sprintf(matrixColumns, "%u", config.matrixColumns);
    sprintf(matrixRows, "%u", config.matrixRows);
    sprintf(matrixLayout, "%u", config.matrixLayout);
    ...
    ESP_LOGD(TAG, "neopixelMatrixData: %s", neopixelMatrixData);
    ESP_LOGD(TAG, "featherMatrixData: %s", featherMatrixData);
    ESP_LOGD(TAG, "featherMatrixClock: %s", featherMatrixClock);
    ESP_LOGD(TAG, "matrixColumns: %s", matrixColumns);
    ESP_LOGD(TAG, "matrixRows: %s", matrixRows);
    ESP_LOGD(TAG, "matrixLayout: %s", matrixLayout);
    ...
    neopixelMatrixDataParam = new ESPAsync_WMParameter("neopixelMatrixData", "Neopixel matrix data pin", neopixelMatrixData, 3, "config.neopixelMatrixData");
    ...
    matrixColumnsParam = new ESPAsync_WMParameter("matrixColumns", "Matrix columns", matrixColumns, 3, "config.matrixColumns");
    matrixRowsParam = new ESPAsync_WMParameter("matrixRows", "Matrix rows", matrixRows, 3, "config.matrixRows");
    matrixLayoutParam = new ESPAsync_WMParameter("matrixLayout", "Matrix layout", matrixLayout, 3, "config.matrixLayout");
    ...
    wifiManager->addParameter(neopixelMatrixDataParam);
    wifiManager->addParameter(featherMatrixDataParam);
    wifiManager->addParameter(featherMatrixClockParam);
    wifiManager->addParameter(matrixColumnsParam);
    wifiManager->addParameter(matrixRowsParam);
    wifiManager->addParameter(matrixLayoutParam);
  ...
  void updateConfiguration(ESPAsync_WiFiManager* wifiManager) {
    if (safeConfigFlag) {
      ...
      ESP_LOGD(TAG, "neopixelMatrixData: %s", neopixelMatrixDataParam->getValue());
      ...
      ESP_LOGD(TAG, "matrixColumns: %s", matrixColumnsParam->getValue());
      ESP_LOGD(TAG, "matrixRows: %s", matrixRowsParam->getValue());
      ESP_LOGD(TAG, "matrixLayout: %s", matrixLayoutParam->getValue());
      ...
      config.neopixelMatrixData = (uint8_t)atoi(neopixelMatrixDataParam->getValue());
      ...
      config.matrixColumns = (uint8_t)atoi(matrixColumnsParam->getValue());
      config.matrixRows = (uint8_t)atoi(matrixRowsParam->getValue());
      config.matrixLayout = (uint8_t)atoi(matrixLayoutParam->getValue());
    ...
    delete neopixelMatrixDataParam;
    delete featherMatrixDataParam;
    delete featherMatrixClockParam;
    delete matrixColumnsParam;
    delete matrixRowsParam;
    delete matrixLayoutParam;
    delete hub75R1Param;
    delete hub75G1Param;
    delete hub75B1Param;
    delete hub75R2Param;
